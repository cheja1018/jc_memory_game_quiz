...
def instructions():
    ...
    print()
    # the rules
    print("The game objective is to play as many rounds as possible\n"
          "To play, the computer would generate a random 8 digit number.\n"
...
# enter to start new round/timer
def enter_continue():
...
# routine for main part of the game goes here
# until lives reach 0, keep the loop going
while lives > 0:

    enter_continue()  # start timer, continue to next round
    rounds += 1
    print("Round", rounds)
    ...
    for clear_line in range(1, 3):  # clear line by counting up to two when 1 display the gen number, when 2 change gen num to nothing
        if clear_line == 1:
            for i in range(1, 10+1):
                print(f"\r{num}, counting to ten: {i}", end="")  # print num and a timer counting to ten
                sleep(1)
        else:
            print(f"\r", end="")  # delete previous line
